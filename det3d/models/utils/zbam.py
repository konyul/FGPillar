...
from det3d.utils.fuse import fold_conv_bn_eval
class Conv1d(nn.Module):
    ...
    def forward(self, x):
        if self.bn_folding:
            if self.bn==True:
                if not (self.linear.training or self.norm.training):
                    self.linearnorm = fold_conv_bn_eval(self.linear, self.norm)
                    x = self.linearnorm(x)
                else:
                    x = self.linear(x)
                    x = self.norm(x)
...
class Zconv(nn.Module):
    def __init__(self,
                 config):
        super().__init__()
        self.bn_folding = config.bn_folding
        bin_shuffle_list = []
        conv_list = []
        linear_list = []
        self.encoder_levels = config.encoder_level
        self.input_channel = config.input_channel
        ratio = config.get('channel_reduce',None)
        ...
        for encoder_level in self.encoder_levels:
            ...
            if ratio == None or encoder_level == 1:
                inter_channel = 32
            else:
                inter_channel = int(out_channel*ratio)
            in_channel = int(inter_channel*self.num_bins)
            if encoder_level != 1:
                linear_list.append(Conv1d(self.input_channel*(2**(encoder_level-1)), inter_channel, bn=False))
            bin_shuffle_list.append(bin_shuffle(in_channel, out_channel))
            conv_list.append(Conv1d(8, out_channel, bn_folding=self.bn_folding))
    ...
    def dyn_voxelization(self, data_dict):
        ...
        points_data = torch.cat([points[:,:4], points_data],dim=-1)
    ...
    def point_sum_sparse(self, data_dict, downsample_level):    
        ...
        unq_inv, batch_idx = torch.sort(unq_inv)
